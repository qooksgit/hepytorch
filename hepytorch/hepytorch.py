...
class HEPTorch():
    ...
    def train(self):
        ...
        target = torch.from_numpy(self.data['y'].values).type(torch.float).reshape(-1, 1)
        target = target.to(self.device)
        train_data = DataLoader(LinearDataset(data, target), batch_size=batch_size, shuffle=True)
